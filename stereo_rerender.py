...
def masked_blur(img, ksize=(6,6), sigma=0):
    ...
    """
    # Build 2D Gaussian kernel (1D then outer‑product → 2D)
    ...
    valid_mask = (~black_mask).astype(np.float32)
    ...
    # 4) Normalize convolution, avoid division by zero
    # for each channel, divide by weight_sum
...
if __name__ == '__main__':
    ...
    # Required depth video input
    parser.add_argument(
        '--depth_video', type=str,
        help='Path to input depth-encoded video file.',
        required=True
    )

    # Optional color video (to overlay color onto depth mesh)
    parser.add_argument(
        '--color_video', type=str,
        help='Path to input color video file.',
        required=False
    )

    # Field-of-view settings: either fixed or per-frame
    parser.add_argument('--xfov', type=float, help='Horizontal FOV in degrees.', required=False)
    parser.add_argument('--yfov', type=float, help='Vertical FOV in degrees.', required=False)
    parser.add_argument(
        '--xfov_file', type=str,
        help='JSON file specifying xfov per frame.',
        required=False
    )

    # Other optional inputs
    parser.add_argument('--max_depth', default=100, type=int, help='Maximum depth encoded in video.')
    ...
    parser.add_argument('--mask_video', type=str, help='Mask video for background infill.')

    # Rendering mode flags
    parser.add_argument('--touchly0', action='store_true', help='Render in touchly0 format (stereo 3D).')
    parser.add_argument('--touchly1', action='store_true', help='Render in touchly1 format (mono+depth).')
    parser.add_argument('--touchly_max_depth', default=5, type=float, help='the max depth that touchly is cliped to', required=False)
    parser.add_argument('--vr180', action='store_true', help='Render in VR180 180° side-by-side.')
    parser.add_argument('--render_as_pointcloud', action='store_true', help='Render output as point cloud.')

    # Infill and mask options
    parser.add_argument('--do_basic_infill', action='store_true', help='Use basic in-house infill algorithm.')
    parser.add_argument('--infill_mask', action='store_true', help='Save infill masks alongside output.')
    parser.add_argument('--remove_edges', action='store_true', help='Remove mesh edges not visible in input frames.')
    parser.add_argument('--dont_place_points_in_edges', action='store_true', help='Skip adding edge points for infill.')

    # Performance and output
    parser.add_argument('--compressed', action='store_true', help='Compress output video (lower quality).')
    parser.add_argument('--max_frames', default=-1, type=int, help='Stop after processing this many frames.')
    ...
    parser.add_argument(
        ...
        help='Enable debug mode: save intermediate left/right/stereo frames as PNG.'
    )

    args = parser.parse_args()

    # Ensure at least one FOV argument is provided
    if args.xfov is None and args.yfov is None and args.xfov_file is None:
        print("Error: Either --xfov_file, --xfov or --yfov must be provided.")
        sys.exit(1)
    ...
    out_writer = cv2.VideoWriter(output_file, codec, frame_rate, out_size)

    # Optional infill mask video writer
    infill_mask_writer = None
    if args.infill_mask:
        infill_mask_writer = cv2.VideoWriter(output_file+"_infillmask.mkv", cv2.VideoWriter_fourcc(*"FFV1"), frame_rate, out_size)
    ...
    left_shift  = -(args.pupillary_distance / 1000.0) / 2.0
    right_shift =  (args.pupillary_distance / 1000.0) / 2.0
    ...
    # Compute total_frames once
    if args.max_frames != -1:
        total_frames = args.max_frames
    else:
        total_frames = int(depth_video.get(cv2.CAP_PROP_FRAME_COUNT))

    # Main processing loop
    while depth_video.isOpened():
        ...
        remaining_time = (elapsed_time / frame_n) * (total_frames - frame_n +1)

        print(f"Processing frame {frame_n:6d}/{total_frames:6d} - Elapsed: {elapsed_time:.0f} seconds ; Remaining: {remaining_time:.0f} seconds", end='\r')

        # Convert encoded depth frame from BGR to numeric depth
        rgb = cv2.cvtColor(depth_frame, cv2.COLOR_BGR2RGB)

        if args.debug:
            raw_R = depth_frame[:,:,2].astype(np.uint32)
            raw_G = depth_frame[:,:,1].astype(np.uint32)
            raw_B = depth_frame[:,:,0].astype(np.uint32)
        ...
        if transformations is None and args.touchly1: #Fast path we can skip the full render pass
            ...
            if infill_mask_writer is not None:
            ...
            if mask_video is not None:
                ...
                points_2_keep = np.intersect1d(unused_indices, bg_mask)
            ...
            if args.touchly1:
                ...
                if convergence_depths is not None:
                    convergence_distance = float(convergence_depths[frame_n-1])
                ...
                #TODO: test this
                # else:
                #     #Testing set convergence to scene frame average depth
                #     convergence_distance = np.mean(depth)
                ...
                # ------------- Output Composition -------------
                # Concatenate left and right (and optionally depth) side-by-side
    ...
    if infill_mask_writer:
        infill_mask_writer.release()
