...
if __name__ == '__main__':
    ...
    mp = np.array(motions_p)

    depth_frames_helper.save_depth_video(np.array(depths_o), args.depth_video + '_megasam.mkv', frame_rate, MODEL_maxOUTPUT_depth, frame_width, frame_height)
    depth_frames_helper.save_grayscale_video(np.array(motions_p), args.depth_video + '_megasam_motion.mkv', frame_rate, 1.0, frame_width, frame_height)
    ...
    fovx, fovy = depth_map_tools.fov_from_camera_matrix(est_cam_matrix)
